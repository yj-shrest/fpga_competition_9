    ...
    always @(posedge clk or negedge rstn) begin
        if (!rstn) begin
            ...
            if (start && !computation_active && !done_reg) begin
                ...
                done_reg <= 0;
                done_delay_counter <= 0;
                //$display("[%0t] MP_Edge_Layer_B7_L3: Computation started", $time);
                ...
                if (done_delay_counter < 1) begin  // Wait cycles for neurons to finish
                    ...
                    done_reg <= 0;
                    // $display("[%0t] MP_Edge_Layer_B7_L3: Waiting for neurons, delay=%0d", 
                    //         $time, done_delay_counter);
                end else begin
                    // After delay, assert done (keep computation_active HIGH!)
                    //$display("[%0t] MP_Edge_Layer_B7_L3: About to assert done - neuron outputs[0]=%h", 
                            //$time, neuron_outputs[0]);
                    //$display("[%0t] MP_Edge_Layer_B7_L3: data_out_flat[31:0]=%h", 
                            //$time, data_out_flat[31:0]);
                    done_reg <= 1;
                    done_delay_counter <= 0;
                    //$display("[%0t] MP_Edge_Layer_B7_L3: Computation completed, asserting done", $time);
                end
                ...
                done_reg <= 0;
                done_delay_counter <= 0;
                //$display("[%0t] MP_Edge_Layer_B7_L3: Restarting computation", $time);
            end
        end
    end
